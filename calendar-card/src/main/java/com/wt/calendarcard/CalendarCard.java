...
public class CalendarCard extends RelativeLayout {
    ...
    public CalendarCard(Context context, Calendar dateDisplay) {
		super(context);
	...
	private void init(Context ctx) {
		...
		cardTitle.setText(new SimpleDateFormat("yyyy-MM", Locale.getDefault()).format(dateDisplay.getTime()));
			...
			for(int x=0; x<row.getChildCount(); x++) {
				...
				cell.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View cell) {
                        ...
                        dateDisplay = item.getDate();

						for(CheckableLayout c : cells) {
                            CardGridItem item2 = (CardGridItem)c.getTag();
                            if(item2==null) break;
                            c.setChecked(DateUtil.isSameDay(item2.getDate()));
                        }

						((CheckableLayout)cell).setChecked(true);
						
						if (getOnCellItemClick()!= null)
							getOnCellItemClick().onCellClick(cell, item); // TODO create item
					}
				});
		...
		mOnItemRenderDefault = new OnItemRender() {
			...
			public void onRender(CheckableLayout v, CardGridItem item) {
				...
				((TextView)layout.getChildAt(0)).setText(item.getDayOfMonth().toString());
			}
		};
    ...
    public void setHeaderVisibility(boolean visible){
        if(visible && cardTitle != null && cardDays != null) {
            cardTitle.setVisibility(View.VISIBLE);
            cardDays.setVisibility(View.VISIBLE);
        }else if(cardTitle != null && cardDays != null){
            cardTitle.setVisibility(View.GONE);
            cardDays.setVisibility(View.GONE);
        }
    }
	...
	private void updateCells() {
		...
		for(int i=firstDay; i<lastDay; i++) {
            ...
            cell.setChecked(DateUtil.isSameDay(date));
			cell.setVisibility(View.VISIBLE);
			(mOnItemRender == null ? mOnItemRenderDefault : mOnItemRender).onRender(cell, item);
			counter++;
		}
